...
async function main() {
  console.log(chalk.cyan.bold('\nPRP Starter Installer (Cursor + Claude Code)'));
  ...
  let srcRoot = path.resolve(__dirname, '..', '..', 'project-structure', 'prp-starter');
  if (!(await fs.pathExists(srcRoot))) {
    srcRoot = path.resolve(__dirname, '..', 'assets', 'prp-starter');
  }
  ...
  if (answers.ides.includes('claude-code')) {
    ...
    // For this starter, install rules under .claude/commands/PRP/{rules}
    const base = path.join(destDir, '.claude', 'commands', 'PRP');
  ...
  console.log(chalk.dim(' - For Claude Code, open the Commands panel and look under PRP/rules.'));
}
